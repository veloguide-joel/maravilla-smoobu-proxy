...
export async function confirmHoldById({ holdId, stripeSessionId, stripePaymentIntentId }) {
  ...
  const result = await pool.query(
    `UPDATE booking_intents
     ...
     RETURNING id, status, stripe_session_id, stripe_payment_intent_id, updated_at`,
    [holdId, sessionId, paymentIntentId]
  );
  if (result.rows.length === 0) {
    throw new Error("Hold not found or already confirmed");
  }
